...
class ProjectControllerTest {
  ...
  void createProjectShouldReturnCreatedAndCallProjectService() throws Exception {
    CreateProjectRequest request =
        new CreateProjectRequest("Cloud Task Manager", "Project for organizing tasks");
    ...
    when(projectService.createProject(anyString(), anyString())).thenReturn(serviceResponse);
  ...
  void getUserProjectsShouldReturnOkAndProjectList() throws Exception {
    List<TaskResponse> projectOneTasks =
        List.of(
            new TaskResponse(
                1L,
                1L,
    ...
    List<TaskResponse> projectTwoTasks =
        List.of(
            new TaskResponse(
                2L,
                2L,
    ...
    List<ProjectResponse> serviceResponse =
        List.of(
            new ProjectResponse(
                1L,
                "Cloud Task Manager",
    ...
    when(projectService.getUserProjects()).thenReturn(serviceResponse);

    mockMvc
        .perform(get("/api/project/user-projects"))
        .andDo(print())
  ...
  void getProjectShouldReturnOkAndProject() throws Exception {
    ...
    when(projectService.getProject(1L)).thenReturn(serviceResponse);

    mockMvc
        .perform(get("/api/project/{projectId}", 1L))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(1))
        .andExpect(jsonPath("$.name").value("Cloud Task Manager"))
