...
import tools.jackson.databind.ObjectMapper; // need to import this for objectMapper to work in
...
class AuthControllerTest {
  ...
  void registerShouldReturnOkAndCallAuthService() throws Exception {
    // Arrange
    RegisterRequest request = new RegisterRequest("kevin@test.com", "Password123!", "Kevin");

    AuthResponse serviceResponse =
        new AuthResponse("User registered successfully ", null, null, null, null);
    ...
    verify(authService).registerUser("Kevin", "kevin@test.com", "Password123!");
  }
  ...
  void loginShouldReturnOkWithInvalidCredentialsMessageWhenServiceRejectsLogin() throws Exception {
    // Arrange
    LoginRequest request = new LoginRequest("kevin@test.com", "wrong-password");

    AuthResponse serviceResponse = new AuthResponse("Invalid credentials", null, null, null, null);

    when(authService.loginUser("kevin@test.com", "wrong-password")).thenReturn(serviceResponse);
  ...
  void logoutShouldReturnOkAndCallAuthService() throws Exception {
    ...
    AuthResponse serviceResponse =
        new AuthResponse("User logged out successfully", null, null, null, null);
  ...
  void refreshShouldReturnOkWithNewTokenAndCallAuthService() throws Exception {
    // Arrange
    RefreshTokenRequest request = new RefreshTokenRequest("refresh-token", "kevin@test.com");
    ...
    when(authService.refresh("refresh-token", "kevin@test.com")).thenReturn(serviceResponse);
