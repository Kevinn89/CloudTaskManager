...
class TaskControllerIntegrationTest extends AbstractWebIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        ownerProject = createProject(
                "Owner Project",
                "Project owned by Kevin",
                owner.getId()
        );

        secondOwnerProject = createProject(
                "Second Owner Project",
                "Another project owned by Kevin",
                owner.getId()
        );

        otherUserProject = createProject(
                "Other User Project",
    ...
    void getTasksShouldReturnOnlyTasksForRequestedProjectOwnedByAuthenticatedUser() throws Exception {
        ...
        TaskEntity expectedTask = createTask(
        ...
        mockMvc.perform(get("/api/task/project/{projectId}", ownerProject.getId())
                        .header("Authorization", "Bearer " + token))
    ...
    @Test
    void updateShouldReturn200AndPersistChangesWhenTaskBelongsToAuthenticatedUserProject() throws Exception {
    ...
    @Test
    void updateShouldReturn404AndLeaveTaskUnchangedWhenTaskBelongsToDifferentProjectEvenIfUserOwnsBoth() throws Exception {
        // Why: proves user cannot update Project B task through Project A request.
        String token = loginAndExtractAccessToken("kevin@test.com", "Password123!");

        TaskEntity taskInSecondProject = createTask(
        ...
        mockMvc.perform(put("/api/task/update")
                ...
                .andExpect(jsonPath("$.message").value("Task not found for project " + ownerProject.getId()));

        TaskEntity unchangedTask = taskRepository.findById(taskInSecondProject.getId())
                .orElseThrow();
    ...
    void updateShouldReturn401AndLeaveTaskUnchangedWhenProjectBelongsToAnotherUser() throws Exception {
        ...
        TaskEntity otherUserTask = createTask(
                "Other user task",
                "Should not update",
                otherUserProject.getId(),
                otherUser.getId()
    ...
    @Test
    void deleteShouldReturn204AndRemoveTaskWhenTaskBelongsToAuthenticatedUserProject() throws Exception {
        ...
        mockMvc.perform(delete("/api/task/{taskId}/project/{projectId}", task.getId(), ownerProject.getId())
                        .header("Authorization", "Bearer " + token))
    ...
    @Test
    void deleteShouldReturn404AndKeepTaskWhenTaskBelongsToDifferentProjectEvenIfUserOwnsBoth() throws Exception {
        ...
        mockMvc.perform(delete("/api/task/{taskId}/project/{projectId}", taskInSecondProject.getId(), ownerProject.getId())
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message").value("Task not found for project " + ownerProject.getId()));
    ...
    void deleteShouldReturn401AndKeepTaskWhenProjectBelongsToAnotherUser() throws Exception {
        ...
        mockMvc.perform(delete("/api/task/{taskId}/project/{projectId}", otherUserTask.getId(), otherUserProject.getId())
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.message").value("Invalid Project"));
